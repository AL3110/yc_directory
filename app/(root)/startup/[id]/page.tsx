// shows new dynamic startup page for each different startup as: /startup/11982
...
const page = async ({ params }: { params: Promise<{ id: string }>}) => {
  ...
  if (!post) return notFound(); // Not found 404 page

  const parsedContent = md.render(post?.pitch || '')

  return (
    <>
    <section className='pink_container !min_h-[230px]'>
      ...
      <div className='space-y-5 mt-10 max-w-4xl mx-auto'>
        <div className='flex-between gap-5'>
          <Link href={`/user/${post.author?._id}`} className='flex gap-2 items-center mb-3'>
            <Image src={post.author.image} alt='avatar' width={64} height={64} className='rounded-full shadow-drop-lg' />
